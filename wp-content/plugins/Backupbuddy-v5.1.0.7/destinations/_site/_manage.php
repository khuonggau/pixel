	...
	<table class="widefat">
		...
		<tbody>
			...
			<tr class="entry-row alternate">
				...
				<td><?php $i = 0; $x = count( $localInfo['activePlugins'] );
				foreach( (array)$localInfo['activePlugins'] as $localPlugin ) {
					$i++;
					echo $localPlugin['name'] . ' v' . $localPlugin['version'];
					if ( $x > $i ) { echo ', '; }
				} ?></td>
				<td><?php $i = 0; $x = count( $deployData['remoteInfo']['activePlugins'] );
				foreach( (array)$deployData['remoteInfo']['activePlugins'] as $remotePlugin ) {
					$i++;
					echo $remotePlugin['name'] . ' v' . $remotePlugin['version'];
					if ( $x > $i ) { echo ', '; }
				} ?> (<?php echo count( $deployData['sendPluginFiles'] ); ?> files to send)</td>
			</tr>
			<tr class="entry-row alternate">
				...
				<td><?php print_r( $deployData['remoteInfo']['activeTheme'] ); ?> (<?php
					if ( $deployData['remoteInfo']['activeTheme'] == $localInfo['activeTheme'] ) {
						echo count( $deployData['sendThemeFiles'] ) . ' files to send';
					} else {
			...
			<tr class="entry-row alternate">
				<td class="tdhead">Media / Attachments</td>
				<td><?php echo $localInfo['mediaCount'] ?></td>
				<td><?php echo $deployData['remoteInfo']['mediaCount']; ?> (<?php echo count( $deployData['sendMediaFiles'] ); ?> files to send)</td>
			</tr>
	</tbody>
	</table>
