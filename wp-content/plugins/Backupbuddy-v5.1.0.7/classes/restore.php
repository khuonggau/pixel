...
class backupbuddy_restore {
	...
	public function swapDatabaseBBSettings() {
		...
		$newPrefix = 'BBnew-' . substr( $this->_state['serial'], 0, 4 ) . '_' . $wpdb->prefix; // Incoming site.
		$oldPrefix = $wpdb->prefix; // Current live site.
		...
		$sql = "SELECT option_value as bb_settings FROM `{$oldPrefix}options` WHERE option_name='pb_backupbuddy';";
		...
		if ( false === $wpdb->query( "UPDATE {$newPrefix}options SET option_value='" . backupbuddy_core::dbEscape( $results['bb_settings'] ) . "' WHERE option_name='pb_backupbuddy';" ) ) {
			return $this->_error( 'Error #372837683: Unable to copy ocer BackupBuddy settings from live site to incoming database in temp table. Details: `' . $wpdb->last_error . '`.' );
		}
