...
class pb_backupbuddy_backup {
	...
	public function deploy_sendContent( $state ) {
		...
		if ( true !== $state['sendMedia'] ) {
			...
			if ( count( $state['sendMediaFiles'] ) > 0 ) { // Media files remain to send.
				$sendFile = array_pop( $state['sendMediaFiles'] ); // Pop off last item in array. Faster than shift.
		...
		if ( true !== $state['sendPlugins'] ) {
			...
			if ( count( $state['sendPluginFiles'] ) > 0 ) { // Plugin files remain to send.
				$sendFile = array_pop( $state['sendPluginFiles'] ); // Pop off last item in array. Faster than shift.
		...
		if ( true !== $state['sendTheme'] ) {
			...
			if ( count( $state['sendThemeFiles'] ) > 0 ) { // Plugin files remain to send.
				$sendFile = array_pop( $state['sendThemeFiles'] ); // Pop off last item in array. Faster than shift.
